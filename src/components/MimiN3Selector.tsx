  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="relative text-center">
        ...
        <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-700 dark:from-emerald-400 dark:via-teal-400 dark:to-indigo-400 bg-clip-text text-transparent">
      ...
      {/* Mode Switcher Tabs */}
      <div className="bg-slate-200/70 dark:bg-slate-800/70 p-1.5 rounded-2xl flex items-center max-w-md mx-auto">
        <button
          ...
          <div className="glass-panel rounded-2xl p-4 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="px-3.5 py-2 text-xs font-bold rounded-lg bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 transition-all flex items-center gap-1.5 cursor-pointer"
              >
                <CheckSquare size={14} />
              ...
              <button
                ...
                className="px-3.5 py-2 text-xs font-bold rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all flex items-center gap-1.5 cursor-pointer"
              >
                <Square size={14} />
                Bỏ chọn hết
              </button>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center justify-between ${
                    isAllSelected
                      ? 'bg-emerald-50/60 dark:bg-emerald-950/40 border-emerald-300 dark:border-emerald-700 shadow-sm'
                      : isSomeSelected
                ...
                >
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={() => {}}
                      className="rounded border-slate-300 dark:border-slate-600 text-emerald-600 focus:ring-emerald-500 h-4.5 w-4.5 cursor-pointer accent-emerald-600"
                    />
                    <div>
                      <h4 className="text-sm font-extrabold text-slate-800 dark:text-slate-100">{lesson.title}</h4>
                      <span className="text-xs font-semibold text-slate-400 dark:text-slate-500">{wordCount} từ vựng</span>
                    </div>
                  </div>

                  <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-100/70 dark:bg-emerald-900/40 px-2.5 py-1 rounded-full">
                    Bài {lesson.id}
                  </span>
                </div>
              );
      ...
      {selectMode === 'range' && (
        <div className="space-y-6">
          <div className="glass-panel rounded-2xl p-6 md:p-8 space-y-6 max-w-3xl mx-auto border border-emerald-100 dark:border-emerald-800 shadow-lg">
            <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-300 font-extrabold text-lg">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 dark:text-slate-100 font-bold text-base"
                />
              </div>
            ...
            <div className="p-4 bg-emerald-50 dark:bg-emerald-950/40 rounded-xl border border-emerald-200 dark:border-emerald-800 text-center">
              <span className="text-sm font-bold text-emerald-900 dark:text-emerald-200">
                Sẽ học <span className="text-emerald-700 dark:text-emerald-400 text-xl font-black">{selectedWordList.length}</span> từ (Từ thẻ #{fromIndex} đến #{toIndex})
              </span>
            </div>
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer border ${
                        isCurrent
                          ...
                          : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 hover:border-emerald-200 dark:hover:border-emerald-800'
                      }`}
                    >
                      {preset.label}
                    </button>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-md overflow-hidden transition-all">
          ...
          <div className="p-4 md:p-6 space-y-4 bg-slate-50/50 dark:bg-slate-900/50">
            ...
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-xs font-medium text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
              {previewSearch && (
                <button
                  onClick={() => setPreviewSearch('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded"
                >
                  Xóa
                </button>
              )}
            ...
            {filteredPreviewList.length === 0 ? (
              ...
              <div className="overflow-x-auto max-h-[360px] overflow-y-auto border border-slate-200 dark:border-slate-700 rounded-2xl bg-white dark:bg-slate-900 shadow-inner scrollbar-thin">
                <table className="w-full text-left text-xs border-collapse">
                  <thead className="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-extrabold uppercase text-[10px] tracking-wider sticky top-0 z-10">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700 font-medium">
                    {filteredPreviewList.map((entry) => (
                      <tr key={entry.item.id} className="hover:bg-emerald-50/40 dark:hover:bg-emerald-950/40 transition-colors">
                        <td className="py-2.5 px-4 text-center">
                          <span className="inline-block px-2 py-0.5 rounded-md bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 font-black text-[11px]">
                            #{entry.globalNum}
                          </span>
                        </td>
                        <td className="py-2.5 px-4 font-black text-slate-900 dark:text-slate-100 text-sm">
                          {entry.item.term}
                        </td>
                        <td className="py-2.5 px-4 font-bold text-emerald-700 dark:text-emerald-400">
                          {entry.item.reading || '-'}
                        </td>
                        <td className="py-2.5 px-4 text-slate-700 dark:text-slate-200 leading-snug">
                          {entry.item.meaning || entry.item.answer}
                        </td>
                        <td className="py-2.5 px-4 text-right text-slate-400 dark:text-slate-500 font-bold text-[11px]">
                          Bài {entry.lessonId}
                        </td>
                      </tr>
                    ))}
        ...
        <button
          ...
          className={`px-10 py-4 rounded-2xl font-black text-lg text-white shadow-xl flex items-center gap-3 transition-all duration-300 ${
            selectedWordList.length > 0
              ? 'bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 hover:from-emerald-700 hover:to-teal-800 shadow-emerald-200 dark:shadow-none active:scale-98 cursor-pointer'
