...
export const MistakeNotebook: React.FC<MistakeNotebookProps> = ({
  ...
  const tabClass = (active: boolean) =>
    `flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-extrabold border transition-all cursor-pointer ${
      active
        ? 'bg-rose-600 text-white border-rose-600 shadow-md shadow-rose-100 dark:shadow-none'
    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="relative text-center mb-6">
        <button
          ...
          className="sm:absolute left-0 top-1/2 sm:-translate-y-1/2 mb-4 sm:mb-0 inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 hover:border-rose-200 dark:hover:border-rose-800 text-xs font-extrabold shadow-sm transition-all cursor-pointer"
        >
          <ArrowLeft size={16} />
          <span>Trang chủ</span>
        </button>
        <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-rose-600 via-red-600 to-orange-600 dark:from-rose-400 dark:via-red-400 dark:to-orange-400 bg-clip-text text-transparent">
          Sổ Tay Câu Sai
        </h1>
      ...
      {tab === 'jlpt' ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-12 text-center">
          ...
          <h3 className="text-lg font-extrabold text-slate-800 dark:text-slate-100 mb-1">Sổ tay đang trống</h3>
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
              <p className="text-2xl font-black text-rose-600 dark:text-rose-400">{rows.length}</p>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400">Câu từng sai</p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
              <p className="text-2xl font-black text-orange-600 dark:text-orange-400">{leechCount}</p>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400">Thẻ cứng đầu (leech)</p>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
              <p className="text-2xl font-black text-slate-800 dark:text-slate-100">
                {rows.reduce((acc, r) => acc + r.card.wrong, 0)}
              </p>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400">Tổng lượt sai</p>
            </div>
            <button
              onClick={() => onStartReview(subjectFilter)}
              className="rounded-2xl bg-gradient-to-r from-rose-600 to-red-600 text-white p-4 font-extrabold text-sm flex items-center justify-center gap-2 shadow-lg shadow-rose-100 dark:shadow-none hover:from-rose-700 hover:to-red-700 active:scale-95 transition-all cursor-pointer"
            >
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-rose-200 dark:focus:ring-rose-800 focus:border-rose-300 dark:focus:border-rose-600 transition-all"
              />
            </div>
            <select
              value={subjectFilter}
              onChange={(e) => setSubjectFilter(e.target.value)}
              className="px-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-sm font-bold text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-200 dark:focus:ring-rose-800 cursor-pointer"
            >
          ...
          {filteredRows.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-10 text-center text-sm text-slate-500 dark:text-slate-400 font-semibold">
            ...
            <div className="flex flex-col gap-3">
              {filteredRows.map(({ key, entry, card }) => {
                ...
                return (
                  <div
                    key={key}
                    className={`bg-white dark:bg-slate-800 rounded-2xl border p-4 flex flex-col sm:flex-row sm:items-center gap-3 justify-between transition-all ${
                      leech ? 'border-orange-300 dark:border-orange-700 bg-orange-50/30 dark:bg-orange-950/20' : 'border-slate-200 dark:border-slate-700'
                    }`}
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <span className="text-[10px] font-extrabold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-2 py-0.5 rounded-full border border-indigo-100 dark:border-indigo-800">
                          {entry.subjectTitle}
                        </span>
                        ...
                        {leech && (
                          <span className="text-[10px] font-black uppercase tracking-wider text-orange-800 dark:text-orange-300 bg-orange-100 dark:bg-orange-900/40 px-2 py-0.5 rounded-full border border-orange-200 dark:border-orange-800 flex items-center gap-1">
                            <Flame size={10} />
                            Leech
                          </span>
                        )}
                      ...
                      <p className="font-bold text-slate-800 dark:text-slate-100 text-sm leading-snug truncate">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold mt-0.5 truncate">
                        Đáp án: <span className="text-emerald-700 dark:text-emerald-400 font-bold">{item.meaning || item.answer}</span>
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                      <div className="text-right">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500">{formatInterval(card)}</p>
                      </div>
                    </div>
                  </div>
                );
          ...
          <p className="mt-6 text-center text-xs text-slate-400 dark:text-slate-500 font-semibold flex items-center justify-center gap-1.5">
            <Trash2 size={12} />
