        ...
        {showAccount && (
          ...
          >
            <form
              ...
              className="bg-white dark:bg-slate-800 rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-100 dark:border-slate-700 flex flex-col gap-4"
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">Tài khoản {user.username}</h3>
              ...
              <div className="space-y-3 pt-1">
                <p className="text-xs font-black text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
                  <KeyRound size={14} className="text-indigo-500 dark:text-indigo-400" />
                  Đổi mật khẩu
                </p>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-2xl border-2 border-slate-200 bg-white text-slate-800 placeholder-slate-400 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-400 dark:focus:border-indigo-500 text-sm font-semibold"
                />
                <input
                  type="password"
                ...
                {accountNote && (
                  <p
                    className={`text-xs font-bold flex items-start gap-1.5 ${
                      accountNote.ok ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'
                    }`}
                  >
                    {accountNote.ok ? (
              ...
              <button
                ...
                className="w-full py-2.5 rounded-2xl bg-slate-100 text-slate-600 font-bold text-sm hover:bg-rose-50 hover:text-rose-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-rose-950/40 dark:hover:text-rose-400 transition-colors cursor-pointer inline-flex items-center justify-center gap-1.5"
              >
                <LogOut size={14} />
                Đăng xuất
              </button>
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            {isRegister && signupCodeRequired && (
              <div className="space-y-1">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-2xl border-2 border-slate-200 bg-white text-slate-800 placeholder-slate-400 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-400 dark:focus:border-indigo-500 text-sm font-semibold"
                />
                <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 px-1">
            ...
            {!isRegister &&
              (forgotShown ? (
                <p className="text-[11px] font-semibold text-slate-500 bg-amber-50 border border-amber-200 dark:text-slate-300 dark:bg-amber-950/40 dark:border-amber-800 rounded-2xl px-3 py-2.5 leading-relaxed">
                  <span className="font-black text-amber-800 dark:text-amber-300">Ngu quên mật khẩu thì phải chịu 🤷</span>
                  <br />
                ...
                <button
                  ...
                  className="text-xs font-bold text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 cursor-pointer"
                >
                  Quên mật khẩu?
                </button>
              ))}
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 text-xs font-bold text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 cursor-pointer"
            >
              {isRegister ? (
                <>
                  <LogIn size={13} />
